...
export default function App() {
  ...
  const selectedProduct = t.products.find((p) => p.id === selectedId) || t.products[0];

  const cartItems = useMemo(() => {
    return cart
      .map((item) => {
        const product = t.products.find((p) => p.id === item.id);
    ...
    <div className="min-h-screen bg-gradient-to-b from-pink-50 via-rose-50 to-white text-zinc-800">
      ...
      <header className="sticky top-0 z-50 border-b border-pink-100 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 md:px-8">
          ...
          <nav className="hidden items-center gap-6 text-sm font-medium md:flex">
            <a href="#shop" className="transition hover:text-pink-500">{t.nav.shop}</a>
            <a href="#featured" className="transition hover:text-pink-500">{t.nav.featured}</a>
            <a href="#about" className="transition hover:text-pink-500">{t.nav.about}</a>
            <a href="#faq" className="transition hover:text-pink-500">{t.nav.faq}</a>
            <a href="#contact" className="transition hover:text-pink-500">{t.nav.contact}</a>
          </nav>
      ...
      <main id="top">
          ...
          <div className="relative mx-auto grid max-w-7xl gap-12 px-4 py-16 md:grid-cols-2 md:px-8 md:py-24">
            <div className="flex flex-col justify-center">
              ...
              <div className="mt-8 flex flex-wrap gap-4">
                <a href="#shop" className="rounded-2xl bg-pink-500 px-6 py-3 font-semibold text-white shadow-lg shadow-pink-200 transition hover:-translate-y-0.5">
                  {t.heroPrimary}
                </a>
                <a href="#featured" className="rounded-2xl border border-pink-200 bg-white px-6 py-3 font-semibold text-pink-600 transition hover:bg-pink-50">
                  {t.heroSecondary}
                </a>
              </div>
            </div>
        ...
        <section className="mx-auto grid max-w-7xl gap-4 px-4 py-6 md:grid-cols-3 md:px-8">
          {t.trustBar.map((item) => (
            <div key={item} className="rounded-2xl bg-white p-4 text-center text-sm font-semibold text-zinc-700 shadow-sm ring-1 ring-pink-100">
              {item}
            </div>
          ))}
        </section>
        ...
        <section id="shop" className="mx-auto max-w-7xl px-4 py-16 md:px-8">
          <h2 className="mb-8 text-3xl font-black text-zinc-900">{t.categoriesTitle}</h2>
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {t.categories.map((category) => (
              <button key={category.id} type="button" className="group rounded-[2rem] bg-white p-6 text-left shadow-sm ring-1 ring-pink-100 transition hover:-translate-y-1 hover:shadow-lg">
        ...
        <section id="featured" className="bg-white py-16">
          <div className="mx-auto max-w-7xl px-4 md:px-8">
            <h2 className="mb-8 text-3xl font-black text-zinc-900">{t.featuredTitle}</h2>
            <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
              {t.products.map((product) => (
                <article key={product.id} className="overflow-hidden rounded-[2rem] bg-pink-50 shadow-sm ring-1 ring-pink-100 transition hover:-translate-y-1 hover:shadow-xl">
                  ...
                  <div className="p-5">
                    <div className="text-xs font-semibold uppercase tracking-wide text-pink-400">{product.category}</div>
                    <h3 className="mt-2 text-lg font-bold text-zinc-900">{product.name}</h3>
                    <p className="mt-2 text-sm leading-6 text-zinc-500">{product.desc}</p>
                    <div className="mt-3 text-base font-extrabold text-pink-600">{formatISK(product.price)}</div>
                    <div className="mt-5 flex gap-3">
                      <button onClick={() => addToCart(product.id)} className="flex-1 rounded-2xl bg-pink-500 px-4 py-3 font-semibold text-white transition hover:bg-pink-600">
                        {t.addToCart}
                      </button>
                      <button onClick={() => setSelectedId(product.id)} className="rounded-2xl border border-pink-200 bg-white px-4 py-3 font-semibold text-pink-600 transition hover:bg-pink-50">
                        {t.viewDetails}
                      </button>
                    </div>
                  </div>
          ...
          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
            <article className="overflow-hidden rounded-[2rem] bg-white shadow-sm ring-1 ring-pink-100">
              <div className="grid md:grid-cols-2">
                <img src={selectedProduct.image} alt={selectedProduct.name} className="h-full min-h-[340px] w-full object-cover" />
                <div className="p-8">
                  <div className="inline-flex rounded-full bg-pink-100 px-3 py-1 text-xs font-bold text-pink-600">
                    {selectedProduct.badge}
                  </div>
                  <h2 className="mt-4 text-3xl font-black text-zinc-900">{selectedProduct.name}</h2>
                  <div className="mt-2 text-sm font-semibold uppercase tracking-wide text-pink-400">{selectedProduct.category}</div>
                  <p className="mt-4 text-base leading-7 text-zinc-600">{selectedProduct.desc}</p>
                  <div className="mt-6 text-2xl font-extrabold text-pink-600">{formatISK(selectedProduct.price)}</div>
                  <button onClick={() => addToCart(selectedProduct.id)} className="mt-8 rounded-2xl bg-pink-500 px-6 py-3 font-semibold text-white transition hover:bg-pink-600">
                    {t.addToCart}
                  </button>
                </div>
              </div>
            ...
            <aside className="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-pink-100">
              ...
              <div className="mt-6 space-y-4">
                {cartItems.length === 0 ? (
                  <div className="rounded-2xl bg-pink-50 p-4 text-sm text-zinc-500">{t.emptyCart}</div>
                ) : (
                  cartItems.map((item) => (
                    <div key={item.id} className="rounded-2xl bg-pink-50 p-3">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="text-xs text-zinc-500">{formatISK(item.price)}</div>
                        </div>
                        <div className="text-sm font-bold text-pink-600">{formatISK(item.price * item.qty)}</div>
                      </div>
                      <div className="mt-3 flex items-center justify-between">
                        <div className="text-xs font-semibold text-zinc-500">{t.quantity}: {item.qty}</div>
                        <div className="flex gap-2">
                          <button onClick={() => updateQty(item.id, -1)} className="rounded-xl border border-pink-200 bg-white px-3 py-1 text-sm font-bold text-pink-600">−</button>
                          <button onClick={() => updateQty(item.id, 1)} className="rounded-xl border border-pink-200 bg-white px-3 py-1 text-sm font-bold text-pink-600">+</button>
                        </div>
                      </div>
                    </div>
              ...
              <div className="mt-6 flex items-center justify-between border-t border-pink-100 pt-6">
                <div className="text-sm font-semibold text-zinc-500">{t.subtotal}</div>
                <div className="text-xl font-black text-zinc-900">{formatISK(subtotal)}</div>
              </div>
              <button className="mt-6 w-full rounded-2xl bg-pink-500 px-4 py-4 font-bold text-white transition hover:bg-pink-600">
                {t.checkout}
              </button>
              <button className="mt-3 w-full rounded-2xl border border-pink-200 bg-white px-4 py-4 font-bold text-pink-600 transition hover:bg-pink-50">
                {t.continueShopping}
              </button>
            </aside>
          </div>
        ...
        <section id="about" className="mx-auto max-w-7xl px-4 py-16 md:px-8">
          <h2 className="mb-8 text-3xl font-black text-zinc-900">{t.aboutTitle}</h2>
          <div className="grid gap-6 md:grid-cols-3">
            {t.aboutCards.map((item, i) => (
              <div key={item} className="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-pink-100">
        ...
        <section id="faq" className="bg-white py-16">
          <div className="mx-auto max-w-5xl px-4 md:px-8">
            <h2 className="mb-8 text-center text-3xl font-black text-zinc-900">{t.faqTitle}</h2>
            <div className="space-y-4">
              {t.faqs.map((item) => (
                <details key={item.q} className="rounded-[2rem] bg-pink-50 p-6 ring-1 ring-pink-100">
                  <summary className="cursor-pointer list-none text-lg font-bold text-zinc-900">{item.q}</summary>
        ...
        <section className="bg-gradient-to-r from-pink-400 to-rose-300 py-16 text-white">
          <div className="mx-auto max-w-4xl px-4 text-center md:px-8">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-base text-pink-50 md:text-lg">{t.newsletterDesc}</p>
            <form className="mx-auto mt-8 flex max-w-xl flex-col gap-3 sm:flex-row" onSubmit={(e) => e.preventDefault()}>
              <input type="email" placeholder={t.emailPlaceholder} className="w-full rounded-2xl border-0 px-5 py-4 text-zinc-800 outline-none" />
        ...
        <div className="mx-auto grid max-w-7xl gap-6 px-4 py-10 text-sm text-zinc-500 md:grid-cols-3 md:px-8">
          ...
          <div>
            ...
            <div className="mt-2 flex flex-col gap-2">
              <a href="#shop" className="hover:text-pink-500">{t.nav.shop}</a>
              <a href="#faq" className="hover:text-pink-500">{t.nav.faq}</a>
              <a href="#about" className="hover:text-pink-500">{t.nav.about}</a>
            </div>
          </div>
        </div>
      </footer>
